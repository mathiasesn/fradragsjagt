...
def test_flagger_boernebidrag_naar_profil_angiver_det():
    profil = _profil(betaler_boernebidrag=True)
    oplysninger = Skatteoplysninger()

    forslag = find_oversete_fradrag(oplysninger, profil)

    navne = [f.navn for f in forslag]
    assert any("ørnebidrag" in n for n in navne)
    for f in forslag:
        assert f.kilde


def test_ingen_boernebidrag_forslag_uden_profilflag():
    profil = _profil()
    oplysninger = Skatteoplysninger()
    ...
    navne = [f.navn for f in forslag]
    assert not any("ørnebidrag" in n for n in navne)


def test_ingen_boernebidrag_forslag_hvis_allerede_indberettet():
    profil = _profil(betaler_boernebidrag=True)
    oplysninger = Skatteoplysninger(boernebidrag=15_000.0)
