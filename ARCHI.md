...
- **`TidligAarsopgoerelse`** — output of `projicer_aarsopgoerelse`: stores only `samlet_beregnet_skat` and `indbetalt_skat`; `difference`, `er_restskat` and `beloeb` are derived `@property`s, so the instance cannot be inconsistent. `projicer_aarsopgoerelse` returns `None` when the grounds are insufficient (requires both `a_skat_indeholdt` and `am_bidrag_indeholdt`), mirroring how the report already degrades on `beregning is None`.
